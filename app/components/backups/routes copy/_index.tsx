...
import { SkidooImageIndex, SeadooImageIndex, BmwImage, SpyderImageIndex, SuzukiImageindex, KawasakiImageIndex, SwitchImageIndex, KawasakiImage } from './logos/logos'
...
import Overview2 from './images/overvview2.png'
...
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger, Input, Button, TooltipProvider, Tooltip, TooltipTrigger, TooltipContent, } from "~/components/ui/index"
...
import { redirectIfLoggedInLoader } from '~/utils/misc.user.server'
import { getSession, authSessionStorage } from "../sessions/auth-session.server";
...
export function Header() {
  ...
  useEffect(() => {
    const targetText = "It sounds rediculous, I know... but you can test how much time your current crm wastes. Send a mass email to 20 people and time how long it takes to, 1: complete the call along with any notes or anything else you need to put in. 2: schedule the follow-up call, properly, leaving yourself good notes to have a great follow up and not have to dig through previous calls to get a sense of where the customer is at. You can cheat this and say it doesnt take long. On average it would take 1:30 to 2:00 each call, unless your driven and good with the crm you have, you can get it down to a min. If your slow on the computer, this number sky rockets horribly.";
    let index = 0;

    const intervalId = setInterval(() => {
      if (index < targetText.length) {
        setText((prevText) => prevText + targetText[index]);
        index++;
      } else {
        clearInterval(intervalId);
      }
    }, 100); // Change this value to speed up or slow down the typing
    ...
    <div className="bg-black">
      <header className="absolute inset-x-0 top-0 z-50">
        <nav className="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
          <div className="flex lg:flex-1">
            <Link to="/" className="-m-1.5 p-1.5">
              <span className="sr-only">Dealer Sales Assistant</span>
          ...
          <div className="flex gap-x-12">
            {navigation.map((item) => (
              <a key={item.name} href={item.href} className="cursor-pointer text-sm font-semibold leading-6 text-slate1">
                {item.name}
              </a>
            ))}
          </div>
            ...
            {user ? (
              <>
                <Link
                  ...
                  className="-mx-3 cursor-pointer  block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate1 hover:bg-gray-50 mr-3"
                >
                  Profile
                </Link>
                <Link
                  to="/logout"
                  className="-mx-3 cursor-pointer  block rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate1 hover:bg-gray-50"
                >
                  Log out
                </Link>
              </>
              ...
              <>
                <Link
                  to="/login"
                  className="-mx-3 block cursor-pointer  rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate1 hover:bg-gray-50 mr-3"
                >
                  Log in <span aria-hidden="true">&rarr;</span>
                </Link>
                <Link
                  to="/register"
                  className="-mx-3 block cursor-pointer  rounded-lg px-3 py-2.5 text-base font-semibold leading-7 text-slate1 hover:bg-gray-50"
                >
        ...
        <Dialog as="div" className="" open={mobileMenuOpen} onClose={setMobileMenuOpen}>
          ...
          <Dialog.Panel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-slate1/10">
                ...
                <div className="space-y-2 py-6">
                  {navigation.map((item) => (
                    <a
                      key={item.name}
                      href={item.href}
                      className="-mx-3 block cursor-pointer  rounded-lg px-3 py-2 text-base font-semibold leading-7 text-slate1 hover:bg-gray-50"
                    >
                      {item.name}
                    </a>
                  ))}
        ...
        <div className="mx-auto max-w-2xl py-32 sm:py-48 lg:py-56">
          ...
          <div className="text-center">
            ...
            <div className='mt-[295px]'>
              ...
              <video alt="logo" width='450' height='450' src={Indexvideo} className='mt-[50px]' />
            </div>
