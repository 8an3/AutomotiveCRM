...
export default function UploadFile() {
  const { UploadedDocs, finance, merged } = useLoaderData();
      ...
      >
        <div className="grid grid-cols-2">
          ...
          <Select name='category'>
            <SelectTrigger className="w-auto ml-3 mt-5 focus:border-[#60b9fd] text-slate1 border border-slate1 font-bold uppercase">
        ...
        <Button
          ...
          type="submit"
          className={` cursor-pointer  p-3 mt-3 hover:text-[#02a9ff] hover:border-[#02a9ff] text-slate1 border border-slate1 font-bold uppercase text-xs rounded shadow hover:shadow-md outline-none focus:outline-none ease-linear transition-all text-center duration-150 `}
        >
          Upload
        </Button>
      </Form>
      ...
      <div className='ml-2'>
        {UploadedDocs.map((document) => (
          <div key={document.id} className="flex ">
            ...
            <a className='hover:text-[#02a9ff] hover:underline cursor-pointer ml-2' href={`http://localhost:3000/uploads/${document.fileName}`} download> Download</a>
          </div>
        ))}
      </div>
    </div>
