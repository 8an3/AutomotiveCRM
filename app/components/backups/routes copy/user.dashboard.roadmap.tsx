...
export function SettingsLayout() {
	return (
		<>
			...
			<body className="roadmap h-full ">
				<div className="hidden space-y-3 p-10 pb-16 md:block ">
...
/*
this can be nice to copy paste clients email from dashboard
<DropdownMenu>
...
export default function Roadmap() {
	...
	const todoRoadmap = [
		...
		{ type: "campaigns", desc: "https://developers.klaviyo.com/en/reference/get_campaigns" },
		{ type: "Auto email", desc: " customer set time before delivery of what to bring" },
		{ type: "Auto email", desc: " auto email at 5, 2.5 months and 30, 7 days before consent expires, 2 years if bought, 6 months if not" },
		{ type: "Auto email", desc: " customer 2 months after pick up to make sure everything is still good" },
		{ type: "dash", desc: "mass email/sms wip" },
		{ type: "dash", desc: "email / sms campaigns" },
		{ type: "dash", desc: "cross platform ad manager, post it once here and push it to different providors" },
		{ type: "dash", desc: "integrate fb msgr through sms" },
		{ type: "admin", desc: "redesign admin page" },
		{ type: "notifications", desc: "upcoming appt" },
		...
		{ type: "quote", desc: "Save form to session incase something happens to connection or if they srep awway from their computer that way whenever you come back the form is filled out the way you left it" },
	...
	const notCompletedNotImportant = [
		...
		{ type: "service tab", desc: "tech should just be aqble to look at his agenda and know what hes doing for the day, he should have access to all the information he needs from his terminal without having to go find anyone and bug them about it and no more paperwork" },
	]
	...
	const HiddenSection = [
		...
		{ name: "dash - API file upload does not work in production folder API is already in use by platfo procidor move API/file upload to dealerapi/fileupload		" },
		{ name: "dash - Hide feature buttons if unavailable " },
		{ name: "dash - Set automations to per person only " },
		{ name: "dash - Send mass email out to every person that bought a bike within the last 2 years to introduce yourself and try to get them to trade in" },
		...
		{ name: "dash - Set to Lost all customers that we cannot contact due to consent two years for people that purchased 6 months for people that did not" },
		...
		{ name: "-- must have -- autos must have status is active  -  result is attempted/reached   -  delay one min out once criteria is met --  -- -set an auto task to create another task if last one wasnt checked after a certain period  -  -vehichle anniversy   -  text trigger on del date  - reminders to follow up if the customer sent email/text appintment show not sold follow up" },
	]
	...
	const organizedTasksnotCompletedNotImportant = {};
	notCompletedNotImportant.forEach((item) => {
		if (!organizedTasksnotCompletedNotImportant[item.type]) {
			organizedTasksnotCompletedNotImportant[item.type] = [];
		}
		organizedTasksnotCompletedNotImportant[item.type].push(item);
	});
			...
			<div className="w-[80%] mx-auto">
				...
				<div className="grid grid-cols-1">
					{user.email === 'skylerzanth@gmail.com' && (
						<>
							<p className="text-picton-blue-50 text-sm text-picton-blue-50">
								hidden section only visible to you
							</p>
							{HiddenSection.map((item) => (
								<div
									key={item.name}
									className="ml-3 flex items-center justify-between mt-2"
								>
									<label className='text-picton-blue-50' htmlFor={item.name}>{item.name}</label>
									<input className="mr-3" type="checkbox" name={item.name} />
								</div>
							))}
							<Separator className="mt-5 mb-5 text-picton-blue-50" />
						</>
					)}
				</div>
				<>
				...
				<>
					<div>
						<h3 className="text-lg font-medium mt-10 text-picton-blue-50">To-Do Not Important</h3>
						<Separator />
						{Object.entries(organizedTasksnotCompletedNotImportant).map(([type, tasks]) => (
							<div key={type}>
								<h4 className='mt-3 ml-3 text-picton-blue-50'>{type}</h4>
								<Separator />
								{tasks.map((task) => (
