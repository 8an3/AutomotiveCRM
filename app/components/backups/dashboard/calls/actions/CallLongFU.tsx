...
import { createClientFinanceApt } from "~/utils/financeAppts/createClientApt.server";


export default async function callLongFU({ formData, today, userId, user, financeId, CompleteLastAppt }) {
  ...
  let followUpDay = await updateFinanceWithDashboard(updatedData)
  ...
  const createFinanceApptData = await createClientFinanceApt(financeData, dashData, email, clientData, financeId, userId)
