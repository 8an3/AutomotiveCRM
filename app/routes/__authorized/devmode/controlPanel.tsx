...
import { Mail } from '~/components/dev/mail/mail'
...
export const loader = async ({ request }: LoaderArgs) => {
  ...
  const mails = await prisma.dSALeads.findMany({})
  return json({ mails, layout, collapsed });

}
export default function DashboardPage() {
  const { mails, layoutCookie, collapsedCookie } = useLoaderData()
  ...
  const renderContent = (selectedContent) => {
    ...
    switch (selectedContent) {
      case `Work In Progres`:
        selectedItems = [getDoneNow, WIP, issue, doneneedstesting]
        break;
      case `Issue`:
        selectedItems = issue
        break;
  ...
  const layout = layoutCookie//cookies().get("react-resizable-panels:layout:mail")
  const collapsed = collapsedCookie//cookies().get("react-resizable-panels:collapsed")
        ...
        <div className="flex-1 space-y-4 p-8 pt-6mt-3 ">
          ...
          <Tabs defaultValue="overview" className="space-y-4">
            <TabsList>
              ...
              <TabsTrigger value="Contact Us Emails">Contact Us Emails</TabsTrigger>
            ...
            <TabsContent value="To-Do" className="space-y-4">
            ...
            <TabsContent value="Contact Us Emails" className="space-y-4">
