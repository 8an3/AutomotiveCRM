  ...
  if (!financeData) {
    ...
    const savedActivix = await prisma.activixLead.create({
      data: {
        ...
        credit_approved: activixData.credit_approved ?? false,
    ...
    try {
      ...
      const activixData = await prisma.activixLead.create({
        data: {
          ...
          credit_approved: formData.credit_approved ?? false,
...
export async function UpdateLeadBasic(formData, user) {
  ...
  const updateLocal = await UpdateLocalLead(financeData)

  return ({ response, updateLocal })
...
export async function UpdateLeadApiOnly(formData, user) {
  ...
  const response = await axios.put(
    `https://api.crm.activix.ca/v2/leads/${activixId}`,
    {
      ...
      birth_date: formData.dob,
      address_line1: formData.address,
      credit_approved: formData.approved ?? false,
      sale_date: formData.sold,
      delivery_date: formData.deliveredDate,
