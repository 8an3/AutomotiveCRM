...
export async function loader({ request, params }: LoaderFunction) {
  ...
  let getData;
  if (dept === 'accessories') {
    getData = await AccDept(page, perPage, skip, take,)

  }
  if (dept === 'all') {
    getData = await AllDept(page, perPage, skip, take)

  }
  if (dept === 'parts') {
    getData = await PartsDept(page, perPage, skip, take)

  }
  // console.log(getData, 'getData')
  return getData
}
async function PartsDept(page, perPage, skip, take) {
  try {
    const allOrders = await prisma.accHandoff.findMany({
      where: {
        sendTo: 'Parts',
    ...
    const totalOrders = await prisma.accHandoff.count({
      where: {
        sendTo: 'Parts',
        sendToCompleted: 'false',
      },
...
async function AccDept(page, perPage, skip, take) {
  try {
    const allOrders = await prisma.accHandoff.findMany({
      where: {
        sendTo: 'Accessories',
...
async function AllDept(page, perPage, skip, take) {
  try {
    ...
    const totalOrders = await prisma.accOrder.count({});
