...
import { UserPlus, Gauge, CalendarPlus, ChevronsLeft, Banknote, Phone, CalendarCheck, ChevronsRight, Truck, ArrowLeft, ArrowDownToDot, ArrowRight } from 'lucide-react';
...
export default function DnDResource() {
  ...
  async function FetchData(id) {
    ...
    console.log(id, 'id')
    try {
      const { data, error } = useSWR(getDomain + `/dealer/api/singleAppt?apptId=${id}`, fetcher)
  ...
  const resourceTitle = (resource) => {
    return (
      <div className='h-[50px] justify-center items-center flex mt-[25px] '>
        ...
        <p className='text-foreground text-center text-2xl my-auto ml-3'>{resource.resourceTitle}</p>
      </div>
    )
      ...
      <div className="large-screen-ui">
        <>
          <div className="h-[75px]  w-auto  border-b border-border bg-background text-foreground">
            <h2 className="ml-[50px] text-2xl font-bold tracking-tight">Sales Calendar</h2>
          </div>
          <div className=" grow">
                  ...
                  <div className='mt-5 grow justify-center'>
                    <div className=' grid grid-cols-1 ' >
                      ...
                      <div>
                        ...
                        >
                          <SelectTrigger className="w-[240px]  mx-auto mt-3">
                            <SelectValue />
                          </SelectTrigger>
...
export async function action({ request }: ActionFunction) {
  ...
  if (intent === "EmailClient") {
    ...
    return json({ saveComms, sendEmail, completeApt, formData, setComs, })
  }
  if (intent === 'addNewApt') {
