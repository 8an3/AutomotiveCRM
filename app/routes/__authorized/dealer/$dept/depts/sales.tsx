...
import { json, type DataFunctionArgs, type ActionFunction, createCookie, type LoaderFunction, redirect, defer } from "@remix-run/node";
...
export async function loader({ params, request }: DataFunctionArgs) {
  ...
  const financeNotes = finance.FinanceNote
  const conversations = finance.Comm
  ...
  return json({ APP_URL, finance, getTemplates, callToken, tableData, convoList, newToken, email, userList, financeNotes, user });
}
export default function Accessories() {
  const { user, } = useLoaderData();

  console.log(user, 'user dept sales')

  return (
    <div>
      <Tabs defaultValue="Dashboard" className='m-4'>
        <TabsList>
          <TabsTrigger value="Dashboard">Dashboard</TabsTrigger>
          <TabsTrigger value="To Order">The Numbers</TabsTrigger>
        </TabsList>
        <TabsContent value="Dashboard" className="w-[85vw] mx-auto mt-5" >
          <MainDashbaord />
        </TabsContent>
...
export async function getData(): Promise<dashBoardType[]> {
  const res = await fetch(`/dealer/api/dashboard/calls/loader/manager`)
  if (!res.ok) {
...
export function MainDashbaord() {
  ...
  const { user, finance, searchData, getTemplates, callToken, tableData, convoList, newToken, email, userList, APP_URL } = useLoaderData();
  ...
  const [getDomain, setGetDomain] = useState(APP_URL);


  const navigation = useNavigation();
  const isSubmitting = navigation.state === "submitting";
  const dataFetcher = (url) => fetch(url).then(res => res.json());
  const { data: swrData } = useSWR(isSubmitting ? getDomain + `/dealer/api/dashboard/calls/loader/manager` : null, dataFetcher, {})
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const data = row.original
        return <div className='grid grid-cols-1  w-[125px]'>
          <p className='text-center'>{data.userName}</p>
          <p className='text-center lowercase'>{data.userEmail}</p>
        </div>
      },
    },
    {
      id: 'financePerson',
      accessorKey: "financePerson",
      header: ({ column }) => (
      ...
      cell: ({ row }) => {
        const data = row.original
        return <div className='grid grid-cols-1  w-[125px]'>
          <p className='text-center'>{data.financeManagerName ? data.financeManagerName : ''}</p>
          <p className='text-center lowercase'>{data.financeManager ? data.financeManager : ''}</p>
        </div>
      },
    },
    {
