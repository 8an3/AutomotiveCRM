...
import { ComposeEmail, ComposeEmailTwo, forwardEmail, replyAllEmail, replyMessage, SendNewEmail } from "~/components/microsoft/GraphService"
import {
...
export function EmailClientTextEditor({ content, subject: subjectInput, to: toInput, app, user, customer, mail }: {
  ...
  async function SendEmail(body) {
    ...
    switch (sendType) {
      case 'Reply':
        sendEmail = ComposeEmailTwo(app.authProvider!, subject, body, to);
        break;
      case 'Reply All':
        // code block
        break;
    ...
    const clientfile = await prisma.clientfile.findUnique({ where: { email: to } })
  ...
  useEffect(() => {
    ...
    if (sendPackage === false) {
      toast.error(`Something went wrong... Did not send email...`, {
        description: sendPackage === false ? 'Need to press send as the last step to send the email.' :
          null,
      })
