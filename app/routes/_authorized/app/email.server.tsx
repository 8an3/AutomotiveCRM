...
import Payments from "~/routes/emails/custom/payments";
import Options from "~/routes/emails/custom/options";
import WPdf from "~/routes/emails/custom/paymentswPdf";
import OptionsWBreakdown from "~/routes/emails/custom/OptionsWBreakdown";
import OptionsWSpec from "~/routes/emails/custom/OptionsWPDF";
import TempPayments from "~/routes/emails/custom/tempPayments";
import TempPaymentsBreakdown from "~/routes/emails/custom/tempPaymentsBreakdown";
import TempPaymentsBreakdownPDF from "~/routes/emails/custom/tempPaymentsBreakdownPDF";
import TempOptions from "~/routes/emails/custom/tempOptions";
import TempOptionsWPDF from "~/routes/emails/custom/tempOptionsWPDF";
import TempOptionsWBreakdown from "~/routes/emails/custom/tempOptionsWBreakdown";
import FullCustom from "~/routes/emails/custom/fullCustom";
...
export async function SendPayments(tokens, user, formData) {
  ...
  const iRate = finance?.iRate || '';
  let body = ''
  let topPart = ''
  let weekly = ''
  let biweekly = ''
  let monthly = ''
  let payments = finance.desiredPayments
  let display = ''
  let afterTax = ''
  let afterDeposit = ''
  switch (payments) {
    case 'Standard Payment':
      ...
      afterTax = onTax || '';
      afterDeposit = String(Number(afterTax) - Number(deposit)) || '';
      break;
  ...
  const TO_ADDRESS = to;
  let EMAIL_CONTENT = ''
  if (formData.template === 'justPayments') {
    EMAIL_CONTENT = `From: ${USER_ID}\r\n` +
      `To: ${TO_ADDRESS}\r\n` +
      `Subject: ${subject}\r\n` +
      `Content-Type: text/html; charset=utf-8\r\n\r\n` +  // Change content type to text/html
      `<html>
  ...
  if (formData.template === 'FullBreakdown') {
    EMAIL_CONTENT = `From: ${USER_ID}\r\n` +
      `To: ${TO_ADDRESS}\r\n` +
      `Subject: ${subject}\r\n` +
      `Content-Type: text/html; charset=utf-8\r\n\r\n` +  // Change content type to text/html
      ...
      <head>
        ...
        <script>
        // Function to hide rows where the value in the second column is 0
        function hideRowIfZero() {
          // Get all rows in the price section
          var rows = document.querySelectorAll('table tr');

          // Loop through each row
          rows.forEach(function(row, index) {
            // Check if the row contains "Price" or "MSRP"
            if (row.textContent.includes('Price') || row.textContent.includes('MSRP')) {
              return; // Skip header rows
            }

            // Get the value of the second column in the row
            var valueCell = row.querySelector('td:nth-child(2)');
            if (!valueCell) {
              return; // Skip if there's no second column
            }
      ...
      <body
        ...
        <div class="container" style="background-color: #ffffff; margin: 0 auto; padding: 20px 0 48px; margin-bottom: 64px;">
          <div class="box" style="padding: 0 48px;">
            ...
            <p style="font-size:18px;line-height:1.4;margin:16px 0;color:#484848">
              I hope this message finds you well. I wanted to express my appreciation for the opportunity to meet with you and
              discuss your upcoming purchase of the ${model2}. It was a pleasure learning about your preferences and
              requirements.
              As promised, I have attached the pricing details for the ${model2} to this email. Please review the payments
              below.
            </p>
            <div>
              ...
              <table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation"
                style="max-width: 660px; margin: 0 auto; padding: 20px 0 48px;">
                <tbody>
                  <tr>
                    <td>
                      <table align="center" width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation">
                        ...
                        <tr>
                        <td colspan="2">
                        <span style="font-size:26px;line-height:1.3;margin:16px 0;font-weight:700;color:#484848;">
                        Vehicle
                      </span>
                          <hr />
                        </td>
                          ...
                          <tr>
                          <td align="center" style="font-size:18px;line-height:1.4;margin:16px 0;color:#484848; height:22px;">
                            Year
                          </td>
                          <td align="center" style="font-size:18px;line-height:1.4;margin:16px 0;color:#484848;height:22px;">
                            ${year}
                          ...
                          <tr>
                            ...
                            <td align="center" style="font-size:18px;line-height:1.4;margin:16px 0;color:#484848height:22px;">
                              ${financeModel}
                            </td>
                          </tr>
                          <tr>
       ...
       <tr>
                        ...
                        Price
                          ...
                          <tr>
                            <td align="center" style="font-size:18px;line-height:1.4;margin:16px 0;color:#484848">
                              MSRP
                            </td>
                            <td align="center" style="font-size:18px;line-height:1.4;margin:16px 0;color:#484848">
                              $${msrp}
              ...
              <p class="paragraph" style="font-size: 16px; line-height: 24px; text-align: left;">
                Thank you for considering us for your needs. Your satisfaction is our top priority, and I look forward to
                assisting you further. I value your interest in our products, and I'm here to assist you every step of the
                way. If you decide to move forward with your purchase, call me right away. Otherwise, I will follow up with
                you in a couple of days to ensure a smooth and timely process. You can reach me via email at ${userEmail} or
                directly on my cell phone at ${userPhone} for any inquiries or to secure your purchase.
              </p>
            </div>
          </div>
      </body>
  ...
  if (formData.template === 'FullBreakdownWOptions') {
    EMAIL_CONTENT = `From: ${USER_ID}\r\n` +
      `To: ${TO_ADDRESS}\r\n` +
      `Subject: ${subject}\r\n` +
      `Content-Type: text/html; charset=utf-8\r\n\r\n` +  // Change content type to text/html
