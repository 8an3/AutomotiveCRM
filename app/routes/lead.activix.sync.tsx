...
async function SyncImport(existsInDatabase, user, data) {
  ...
  if (!existsInDatabase) {
    try {
      ...
      const { financeData, activixData, dashboardData } = await CreateActvix();
      console.log('Data imported successfully:', financeData, activixData, dashboardData);
    } catch (error) {
    ...
    try {
      // Update existing records in the database based on the provided data
      console.log(`Record with id ${data.id} does not exist in the finance database`);
      const formData = data;
      const nameParts = user.username.split(' ');
      const firstName = nameParts[0];
      ...
      async function UpdateActvix() {
        try {
          ...
          const activixData = await prisma.activixLead.update({
            where: { id: formData.theRealActId },
            data: {
              actvixId: data.id.toString(),
              account_id: data.account_id.toString(),
              customer_id: data.customer_id.toString(),
      ...
      const { financeData, activixData, dashboardData } = await UpdateActvix();

      console.log('Data updated successfully:', financeData, activixData, dashboardData);
    } catch (error) {
